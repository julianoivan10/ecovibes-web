...
import { ArrowLeft, Calendar, Tag, ArrowRight, Mail } from "lucide-react";
    ...
    <div className="min-h-screen bg-white text-gray-900 selection:bg-emerald-200">
      <main className="pt-32 relative overflow-hidden">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
          <motion.div initial="hidden" animate="show" variants={staggerContainer} className="text-center max-w-3xl mx-auto">
            <motion.div variants={fadeUp} className="mb-8">
              <Link href="/" className="inline-flex items-center text-sm font-semibold text-emerald-600 hover:text-emerald-700 bg-white px-5 py-2.5 rounded-full transition-all border border-gray-200 shadow-sm hover:shadow-md hover:-translate-y-1">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
          ...
          {articles === undefined ? (
            <div className="py-20 text-center flex flex-col items-center">
               ...
               <p className="text-gray-500 font-bold animate-pulse">Memuat artikel terbaru...</p>
            </div>
          ) : articles.length === 0 ? (
            <div className="py-20 text-center bg-gray-50 rounded-3xl border border-gray-100">
              <p className="text-gray-500 font-bold text-lg">Belum ada artikel yang dipublikasikan.</p>
            </div>
          ) : (
            <motion.div initial="hidden" animate="show" variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {articles.map((post) => (
                <motion.div key={post._id} variants={fadeUp} className="h-full">
                  <Link href={`/blog/${post.slug}`} className="group flex flex-col h-full bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl hover:shadow-emerald-100/40 hover:-translate-y-1.5 transition-all duration-500">
                    
                    {/* COVER IMAGE */}
                    <div className="relative w-full h-56 bg-gray-200 overflow-hidden shrink-0">
                      {/* Kita pakai tag <img> biasa dulu agar tidak error jika Anda menaruh link luar (Unsplash) dari Admin */}
                      <img 
                        src={post.image} 
                        alt={post.title} 
                    ...
                    <div className="flex flex-col grow p-6">
                      ...
                      <div className="pt-5 border-t border-gray-100 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-emerald-600 font-bold text-sm group-hover:text-emerald-700 transition-colors">Baca Artikel</span>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
          ...
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true }} variants={fadeUp} className="bg-linear-to-br from-emerald-50 via-white to-teal-50 py-16 px-6 rounded-3xl border border-emerald-100 text-center shadow-sm">
            <div className="max-w-xl mx-auto">
              <div className="w-14 h-14 bg-emerald-100 rounded-2xl flex items-center justify-center mx-auto mb-6">
                <Mail className="w-6 h-6 text-emerald-600" />
              </div>
              <h3 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-3">Tetap Terhubung dengan EcoVibes</h3>
              ...
              <div className="flex flex-col sm:flex-row items-stretch gap-3 max-w-md mx-auto mb-4">
                <input type="email" placeholder="Masukkan email Anda" className="flex-1 px-5 py-3.5 rounded-full border-2 border-gray-200 focus:outline-none focus:ring-4 focus:ring-emerald-200 focus:border-emerald-400 text-gray-900 text-sm font-medium transition-all bg-white" />
                <button className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-3.5 rounded-full font-bold text-sm shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2">
                  Berlangganan <ArrowRight className="w-4 h-4" />
                </button>
              </div>
              <p className="text-xs text-gray-400 font-medium">Kami menghargai privasi Anda. Tidak ada spam, unsubscribe kapan saja.</p>
            </div>
          </motion.div>
        </section>
